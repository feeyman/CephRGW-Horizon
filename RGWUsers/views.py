...
class IndexView(views.HorizonTemplateView):
    template_name = 'ceph/RGWUsers/index.html'
    page_title = _("RGWUsers")

    def get_context_data(self, **kwargs):
        context = super(IndexView, self).get_context_data(**kwargs)
    	context['users'] = api.ceph.get_cephrgw_user()
        context['userinfo'] = api.ceph.get_cephrgw_usrinfo()
...
def ceph_rgw_buckets(request):
    value = ''
    try:
        value = api.ceph.ceph_rgw_buckets(request)
        # LOG.info("ceph_health_status = %s" % value)
    except:
        # msg = _('Unable to get cluster health.')
        # messages.warning(request,msg)
...
def ceph_rgw_objects(request):
value = ''
try:
    value = api.ceph.ceph_rgw_objects(request)
    # LOG.info("ceph_health_status = %s" % value)
except:
    # msg = _('Unable to get cluster health.')
    # messages.warning(request,msg)
